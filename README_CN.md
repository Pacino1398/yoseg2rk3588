...
适合在 x86 开发机上快速验证分割、路径规划和可视化是否一致。实时链路会直接在原始画面上叠加障碍物、目标点和路径，而不是单独生成 Matplotlib 风格规划图。
...
这是当前仓库最接近最终落地的部署形态：只导出一个 `.rknn` 分割模型，后处理、栅格化、路径规划和实时画面叠加仍在 CPU 侧 Python 中执行。
...
如果需要本地窗口显示（`--display local`、`--display both` 或兼容参数 `--view`），把 `opencv-python-headless` 换成 `opencv-python`。
...
然后在上位机浏览器打开：`http://<板端IP>:8080/stream.mjpg`
