...
REALTIME_OVERLAY_ALPHA = 0.35
REALTIME_TRAVERSABLE_ALPHA = 0.45
REALTIME_PATH_COLOR_BGR = (0, 255, 0)
REALTIME_START_COLOR_BGR = (255, 0, 0)
REALTIME_GOAL_COLOR_BGR = (0, 0, 255)
REALTIME_TRAVERSABLE_COLOR_BGR = (88, 255, 88)
REALTIME_TEXT_DARK_BGR = (0, 0, 0)
REALTIME_TEXT_LIGHT_BGR = (255, 255, 255)
REALTIME_INFO_BG_BGR = (245, 245, 245)
REALTIME_INFO_BORDER_BGR = (140, 140, 140)


def _cell_bounds(cell: tuple[int, int], grid_scale: int, canvas_shape: tuple[int, int]) -> tuple[tuple[int, int], tuple[int, int]]:
    canvas_h, canvas_w = canvas_shape
    x1 = max(0, int(cell[0] * grid_scale))
    y1 = max(0, int(cell[1] * grid_scale))
    x2 = min(canvas_w - 1, x1 + grid_scale - 1)
    y2 = min(canvas_h - 1, y1 + grid_scale - 1)
    return (x1, y1), (x2, y2)


def _draw_overlay_cells(frame: np.ndarray, grid_handler: GridMapHandler, grid_scale: int) -> np.ndarray:
    result = frame.copy()
    ...
    obstacle_overlay = result.copy()
    for cell in display_obs:
        top_left, bottom_right = _cell_bounds(cell, grid_scale, canvas_shape)
        cv2.rectangle(
            obstacle_overlay,
            top_left,
            bottom_right,
            get_obstacle_color(cell, grid_handler.obstacle_heights),
            thickness=-1,
        )
    result = cv2.addWeighted(obstacle_overlay, REALTIME_OVERLAY_ALPHA, result, 1.0 - REALTIME_OVERLAY_ALPHA, 0)

    if grid_handler.traversable_obstacles:
        traversable_overlay = result.copy()
        for cell in grid_handler.traversable_obstacles:
            top_left, bottom_right = _cell_bounds(cell, grid_scale, canvas_shape)
            cv2.rectangle(
                traversable_overlay,
                top_left,
                bottom_right,
                REALTIME_TRAVERSABLE_COLOR_BGR,
                thickness=-1,
            )
        result = cv2.addWeighted(
            traversable_overlay,
            REALTIME_TRAVERSABLE_ALPHA,
            result,
            1.0 - REALTIME_TRAVERSABLE_ALPHA,
            0,
        )
...
def _draw_path_and_markers(
    ...
    if path:
        path_points = np.asarray([cell_to_pixel(cell, grid_scale) for cell in path], dtype=np.int32)
        if len(path_points) >= 2:
            cv2.polylines(
                frame,
                [path_points.reshape((-1, 1, 2))],
                isClosed=False,
                color=REALTIME_PATH_COLOR_BGR,
                thickness=max(2, grid_scale // 3),
                lineType=cv2.LINE_AA,
            )
        else:
            cv2.circle(frame, tuple(path_points[0]), max(3, grid_scale // 3), REALTIME_PATH_COLOR_BGR, thickness=-1)

    marker_radius = max(4, grid_scale // 2)
    cv2.circle(frame, cell_to_pixel(start, grid_scale), marker_radius, REALTIME_START_COLOR_BGR, thickness=-1, lineType=cv2.LINE_AA)
    cv2.circle(frame, cell_to_pixel(goal, grid_scale), marker_radius, REALTIME_GOAL_COLOR_BGR, thickness=-1, lineType=cv2.LINE_AA)
...
def _draw_class_labels(
    ...
    font_scale = max(0.35, min(0.7, grid_scale / 16.0))
    thickness = max(1, grid_scale // 10)
    ...
    for (x, y), label, text_color in build_class_annotations(
        ...
        center_x = int(round(x * grid_scale))
        center_y = int(round(y * grid_scale))
        (text_w, text_h), baseline = cv2.getTextSize(label, font, font_scale, thickness)
        x1 = max(0, min(frame_w - text_w - 8, center_x - text_w // 2 - 4))
        y1 = max(0, min(frame_h - text_h - baseline - 8, center_y - text_h // 2 - 4))
        x2 = min(frame_w - 1, x1 + text_w + 8)
        y2 = min(frame_h - 1, y1 + text_h + baseline + 8)

        cv2.rectangle(frame, (x1, y1), (x2, y2), REALTIME_INFO_BG_BGR, thickness=-1)
        cv2.rectangle(frame, (x1, y1), (x2, y2), REALTIME_INFO_BORDER_BGR, thickness=1)
        color = REALTIME_TEXT_LIGHT_BGR if text_color == "white" else REALTIME_TEXT_DARK_BGR
        cv2.putText(
            frame,
            label,
            (x1 + 4, y2 - baseline - 4),
...
def _draw_realtime_info(frame: np.ndarray, grid_handler: GridMapHandler, path: list[tuple[int, int]]) -> None:
    info_lines = [
        f"path cells: {len(path)}",
        f"obstacles: {len(grid_handler.obstacles)}",
        f"target: {'yes' if grid_handler.target_point is not None else 'no'}",
    ]
    font = cv2.FONT_HERSHEY_SIMPLEX
    font_scale = 0.55
    thickness = 1
    padding = 8
    line_gap = 6
    sizes = [cv2.getTextSize(line, font, font_scale, thickness)[0] for line in info_lines]
    text_width = max((size[0] for size in sizes), default=0)
    text_height = sum(size[1] for size in sizes) + line_gap * max(len(sizes) - 1, 0)
    x1, y1 = 10, 10
    x2 = min(frame.shape[1] - 1, x1 + text_width + padding * 2)
    y2 = min(frame.shape[0] - 1, y1 + text_height + padding * 2)
    cv2.rectangle(frame, (x1, y1), (x2, y2), REALTIME_INFO_BG_BGR, thickness=-1)
    cv2.rectangle(frame, (x1, y1), (x2, y2), REALTIME_INFO_BORDER_BGR, thickness=1)

    cursor_y = y1 + padding
    for line, (width, height) in zip(info_lines, sizes):
        baseline_y = cursor_y + height
        cv2.putText(
            frame,
            line,
...
def render_plan_on_frame(
    ...
    result = _draw_overlay_cells(frame, grid_handler, grid_scale)
    _draw_path_and_markers(result, path, start, goal, grid_scale)
    if show_labels and class_names is not None:
        _draw_class_labels(result, grid_handler, grid_scale, class_names)
    _draw_realtime_info(result, grid_handler, path)
    return result
